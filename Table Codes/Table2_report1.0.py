...
def calculate_broken_chat_by_officer(chat_df, officer_name, target_date, is_shared_chat, all_employee_folders):
    logging.debug(f"Calculating broken chat for officer '{officer_name}' on date '{target_date}' (Shared Chat: {is_shared_chat})")
    second_officer = None 
    if not is_shared_chat:
        result = calculate_broken_chat(chat_df, officer_name, target_date)
        logging.debug(f"Non-shared chat result for '{officer_name}': {result}")
        return result
    ...
    messages_on_date = chat_df[chat_df['date'].dt.date == target_date]

    if messages_on_date.empty:
        logging.debug("No messages found on target date for shared chat analysis.")
        return 'No'

    last_officer_msg_index = -1
    for i, message in enumerate(messages_on_date.itertuples()):
        if message.sender == officer_name:
            last_officer_msg_index = i
            logging.debug(f"Last message from '{officer_name}' at index {i}: {message.message}")

    if last_officer_msg_index == -1:
        logging.debug(f"No messages from '{officer_name}' on target date.")
        return 'No'

    for message in messages_on_date.iloc[last_officer_msg_index + 1:].itertuples():
        if message.sender.startswith('+'):
            logging.debug(f"Student message after '{officer_name}' last message: {message.message}")
            return ('Yes', second_officer)
        if message.sender in all_employee_folders and message.sender != officer_name:
            logging.debug(f"Another officer '{message.sender}' took over before any student replied.")
            second_officer = message.sender  # Store the name of the second officer
            return ('No', second_officer)

    logging.debug(f"No student replied after last message from '{officer_name}', and no other officer took over.")
    return ('No', second_officer)
...
def identify_shared_chats(chat_df, all_employee_folders, target_date):
    # Convert target_date to a date object if it's not already
    if isinstance(target_date, str):
        target_date = datetime.strptime(target_date, '%Y-%m-%d').date()
    ...
    # Check for messages from multiple employees on the target date
    employee_messages = daily_messages[daily_messages['sender'].isin(all_employee_folders)]
    if len(employee_messages['sender'].unique()) > 1:
        return 'D'  # Return 'D' for shared chat

    return 'N'  # Return 'N' if no shared chats are found
...
def process_chat_file(file_path, report_date, main_directory, team_folder, employee_folder, all_employee_folders):
    ...
    chat_file_identifier = f"{employee_folder}-{chat_name}"  # Unique identifier
    ...
    if chat_file_identifier in processed_chats:
    # Skip if this exact chat file has already been processed for this employee
     return None

    processed_chats.add(chat_file_identifier)
    ...
    chat_df = read_chat_file(file_path)
    shared_chat_status = identify_shared_chats(chat_df, all_employee_folders, report_date - timedelta(days=1))
    
    if shared_chat_status == 'D':
        # Call the new broken chat logic for shared chats
        broken_chat_status, second_officer = calculate_broken_chat_by_officer(chat_df, employee_folder, report_date - timedelta(days=1), True, all_employee_folders)
        logging.debug(f"broken chat status: {broken_chat_status}")
    else:
        # Call the existing broken chat logic for non-shared chats
        broken_chat_status, second_officer = "",""
    ...
    row = {
        ...
        **time_spent,
        ...
        'Duplicate' : shared_chat_status,
        'Broken Chat Duplicate'  : broken_chat_status,
...
def process_team_folders(main_directory, report_date):
    ...
    for team_folder in os.listdir(main_directory):
        ...
        all_employee_folders = [folder for folder in os.listdir(team_path) if os.path.isdir(os.path.join(team_path, folder))]
        for employee_folder in os.listdir(team_path):
            ...
            for chat_file in os.listdir(employee_path):
                ...
                row = process_chat_file(chat_file_path, report_date, main_directory, team_folder, employee_folder, all_employee_folders)
                if row:
                    all_data.append(row)
...
# Save to a CSV file
csv_file_path = 'C:\\Users\\aditya\\OneDrive\\Documents\\Chat-Analyzer-V2/chat_data6.csv'  # Define your desired path and file name
